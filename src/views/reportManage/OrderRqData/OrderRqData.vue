  ...
  <div>
    <div class="page flex-wrap">
      ...
      >
        ...
        <el-row
          v-else-if="
            item.fComputer == 'between' && item.fDataType == 'datetime'
          "
        >
          <el-col :span="11">
            <el-date-picker
              v-model="startData[item.fColumn]"
              type="date"
              placeholder="请输入范围值"
            ></el-date-picker>
          </el-col>
    ...
    >
      ...
      <el-table-column width="80" label="合计">
...
export default {
  data() {
    return {
      tableHeadData: [
        { fColumn: "fDate", fColumnDes: "分拣日期", fDataType: "string" },
        {
          fColumn: "fChengzhongNo",
          fColumnDes: "秤重单号",
          fDataType: "string"
        },
        {
        ...
        { fColumn: "fProductName", fColumnDes: "货品名称", fDataType: "string" }
      ],
      searchData: [], //查询的数据
      searchWhere: [], //搜索条件
      fTableView: "t_OutboundOrderReport1",
      fTableViewData: "v_OutboundOrderReport1",
      currentRow: null, //当前行
      tableData: [], //表格数据
      pageNum: 1, // 当前页数
      pageSize: 10, // 每页条数
      ...
      startData: {
        fDate: updateTime(new Date(new Date().toLocaleDateString()).getTime())
      },
  ...
  methods: {
    ...
    //查询
    async search() {
      if (!this.startData.fDate || !this.endData.fDate) {
        this.$message.warning("请选择要查询的分拣日期!");
      } else {
        this.pageNum=1;
    ...
    async getTableData() {
      ...
      for (let k in data) {
        if (
          ...
          let sort = Number(k.match(/\d+/g).join(''));
  ...
  async created() {
    ...
    let where = [
      { Computer: ">=", DataFile: "fDate", Value: this.startData.fDate },
      { Computer: "<=", DataFile: "fDate", Value: this.endData.fDate }
    ];
