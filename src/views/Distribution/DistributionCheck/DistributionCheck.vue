  ...
  <div>
    <div class="page flex-wrap" style="margin-bottom:0">
      <div style="margin-right:20px">
        ...
        <el-button type="primary" size="mini" class="el-icon-search" @click="getHetonghao">获取合同号</el-button>
      </div>
    </div>
    <div class="flex-wrap" style="margin-left:10px">
      <div style="margin-top: 10px">
        ...
        <!-- <el-button type="primary" size="mini" class="el-icon-setting" @click="reset">重置</el-button> -->
        <el-button
          type="primary"
          size="mini"
...
export default {
  ...
  methods: {
    ...
    async getProductList(val) {
      ...
      let res = await getCntContractHTData(this.supplierCode, val);
    ...
    async reconCiliation() {
      ...
      let isCorrect = this.BatchList.every(child => {
        return child.FSEL == 1;
      });
      if (!isCorrect) {
    ...
    async saveData() {
      ...
      if (!res.State) {
        ...
        this.BatchList = [];
        this.tableData = [];
        return this.$message.success("撤回成功");
      }
    },
    // 手动选中Checkbox
    ...
    async handleClick(row, index) {
      ...
      let res = await saveCntContractHTData(null, UpdateData);
      res = JSON.parse(decryptDesCbc(res, String(this.userDes)));
      loading.close();
      console.log(res);
      if (!res.State) {
