...
export default {
  ...
  methods: {
    submitForm(formName) {
      //收货控制天数=保质期*66%   出库有效期天数=保质期*50%;
      //保质期:fShelfLifeNum  收货控制天数:fShelfLifeMinDays   出库效期天数:fOutboundDays
      this.$refs[formName].validate(async valid => {
        if (valid) {
          let num = this.ruleForm.fShelfLifeNum;
          let minDays = num * 0.66;
          minDays = parseFloat(minDays.toFixed(1));
          let maxDays = num * 0.5;
          maxDays = parseFloat(maxDays.toFixed(1));
          this.$set(this.ruleForm, "fShelfLifeMinDays", minDays);
          this.$set(this.ruleForm, "fOutboundDays", maxDays);
          if (
