...
export default {
  ...
  methods: {
      ...
      if (this.BatchList.length == 0) {
        ...
        for (let i = 0; i < this.BatchList.length; i++) {
          ...
          getdata.forEach(item => {
            if (item.fBatchNo) {
              this.$set(item, "fBatchNo", item.fBatchNo.slice(0, 8));
            }
          });
          this.ItemBackData.push(getdata);
          this.dataCode = this.ItemBackData;

          this.isRender = true;
          setTimeout(() => {
            const style =
              "table td,th,tr {border: 1px #000 solid;font-size: 18px; text-align: center;table-layout: fixed;word-break: break-all; word-wrap:break-word;page-break-inside:avoid}; ";
            PrintJS({
              printable: "toPrint",
              type: "html",
              scanStyles: false,
    ...
    //获取从表回显的数据
    async getSearchItemData(fID, fReceiveNo) {
      let searchWhere = [
        {
          Computer: "=",
          DataFile: "fMstID",
      ...
      if (res.State) {
        ...
        if (data.length != 0) {
          data.forEach(element => {
            this.$set(element, "fReceiveNo", fReceiveNo);
          });
        } else {
          let arr = [{ fReceiveNo: fReceiveNo }];
          data = arr;
        }
