      ...
      <template v-for="(item, index) in tableHeadData">
        ...
        >
          <template slot-scope="scope">
            ...
            <el-input
              v-else-if="item.fColumn == 'fRealOutboundNum'"
              v-model="scope.row[item.fColumn]"
              :maxlength="scope.row[item.fLength]"
              :disabled="rowData.fMstState == 1"
            ></el-input>
            <el-input
              v-else
              v-model="scope.row[item.fColumn]"
...
export default {
  ...
  async created() {
    ...
    this.selectOptions = await this.getType("v_Unit", "fNumUnitName");
    //10;
    this.selectOpts = await this.getType("v_Unit", "fVolumetUnitName");
    // 7;
    this.selectWeight = await this.getType("v_Unit", "fWeightUnitName");
    //9;
  }
};
</script>
